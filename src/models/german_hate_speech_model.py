...
def clean_text(text):
    ...
    # Remove non-alphanumeric characters (but keep German-specific characters)
...
# Apply cleaning to Offensive Speech Dataset
german_offn['clean_text'] = german_offn['text'].apply(clean_text)

# Apply cleaning to Hate Dataset
german_hate['clean_text'] = german_hate['text'].apply(clean_text)

# Apply cleaning to Sentiment Dataset
german_sa['clean_text'] = german_sa['text'].apply(clean_text)

print(german_hate['label'].value_counts())
print(german_offn['label'].value_counts())
print(german_sa['label'].value_counts())
...
multitask_dataset = MultiTaskHateDataset(
    hate_data=pd.concat([german_hate[german_hate['label']=='NOT'][:1200], german_hate[german_hate['label']=='HATE'][:1200]]),          # Balanced Hate Speech Dataset
    offensive_data=pd.concat([german_offn[german_offn['label']=='NOT'][:1200], german_offn[german_offn['label']=='OFFN'][:1117]]),          # Balanced Offn Speech Dataset
    sentiment_data=pd.concat([german_sa[german_sa['label']==-1][:794], german_sa[german_sa['label']==0][:800], german_sa[german_sa['label']==1][:800]]),          # Balanced Sent Analysis Dataset
    # sentiment_data=german_sa,         # Sentiment Dataset
    # offensive_data=german_offn, # Balanced Offensive Dataset
    tokenizer=tokenizer,
    max_len=MAX_LEN
)
...
for epoch in range(EPOCHS):
    ...
    for task in best_f1:
        torch.save(model.state_dict(), f'./models/german/{epoch}.pth')
        if val_f1[task] > best_f1[task]:
            best_f1[task] = val_f1[task]
            checkpoint_path = f'./models/german/best_model_{task}.pth'
...
model_save_path = './models/german/multi_task_xlm_roberta.pth'
...
tokenizer_save_path = './models/german/tokenizer_xlm_roberta'
...
optimizer_save_path = './models/german/optimizer.pth'
torch.save(optimizer.state_dict(), optimizer_save_path)
print(f"Optimizer state_dict saved to {optimizer_save_path}")

# Save the scheduler state
scheduler_save_path = './models/german/scheduler.pth'
